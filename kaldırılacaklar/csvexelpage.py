        ...
        if not dependent_variable or not selected_independent_variables:
            ...
            if st.button("Analizi Gerçekleştir"):
                ...
                if top_10_combinations:
                    combo, r2 = top_10_combinations[0]  # Assuming top_10_combinations now contains only one combination
                    # Generate Polynomial Model
                    selected_terms, r2_score, mae_score, y_pred = reg.generate_polynomial_model(X, y, combo, max_degree)
    ...
    if not dependent_variable or not selected_independent_variables:
        ...
        if st.button("Analizi Gerçekleştir"):
            ...
            if top_10_combinations:
                combo, r2 = top_10_combinations[0]  # Assuming top_10_combinations now contains only one combination
                # Generate Polynomial Model
                selected_terms, r2_score, mae_score, y_pred = generate_polynomial_model(X, y, combo, max_degree)
